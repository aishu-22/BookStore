...
public class BookStoreDaoImpl implements BookStoreDao {
	...
	public boolean reviewExists(int reviewId) {
		String jpql = "from CustomerReview c where c.reviewId=:cId";
		TypedQuery<CustomerReview> query = entityManager.createQuery(jpql, CustomerReview.class);
		query.setParameter("cId", reviewId);
	    List<CustomerReview> existingCat = query.getResultList();
	    if(existingCat.isEmpty()) {
	    	return false;
	    }
	    return true;
	...
	public boolean orderExists(int orderId) {
		String jpql = "from OrderInformation o where o.orderId=:oId";
		TypedQuery<OrderInformation> query = entityManager.createQuery(jpql, OrderInformation.class);
		query.setParameter("oId", orderId);
	    List<OrderInformation> existingCat = query.getResultList();
	    if(existingCat.isEmpty()) {
	    	return false;
	    }
	    return true;
